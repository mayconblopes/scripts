...
FACEBOOK_SKIP_PATH_PREFIXES = (
    "/login", "/recover", "/reg", "/checkpoint", "/api/", "/ajax/",
)
...
def facebook_redirect_target(url):
    """Return the external target from Facebook's l.php redirect wrapper."""
    try:
        parsed = urlparse(url)
    except Exception:
        return None
    if (parsed.hostname or "").lower() not in FACEBOOK_REDIRECT_HOSTS:
        return None
    if parsed.path.lower() != "/l.php":
        return None
    values = parse_qs(parsed.query).get("u", [])
    return normalize_url(unquote(values[0])) if values else None


def is_facebook_noise_url(url):
    if not is_facebook_hostname(hostname(url)):
        return False
    path = (urlparse(url).path or "").lower()
    return path.startswith(FACEBOOK_SKIP_PATH_PREFIXES)
...
class ChromiumEngine:
    ...
    def start(self, headless=True):
        websocket_factory = getattr(websocket, "create_connection", None)
        if not callable(websocket_factory):
            module_path = getattr(websocket, "__file__", "desconhecido")
            print("       [!] Biblioteca WebSocket CDP incompatível ou incorreta.")
            print("       [i] O scanner precisa do pacote Python 'websocket-client'.")
            print(f"       [i] Módulo carregado: {module_path}")
            print("       [i] Corrija no Termux com:")
            print("           python -m pip uninstall -y websocket")
            print("           python -m pip install -U websocket-client")
            print("       [i] Verifique também se não existe um arquivo local chamado websocket.py.")
            return False
        if self.is_running():
    ...
    def relaunch_visible(self):
        """Restart our Chromium instance without headless mode for manual login."""
        if self.process and self.started_by_us:
            self.close()
        elif self.is_running():
            print("       [i] Já existe um Chromium/CDP ativo; use essa janela para o login.")
            return True
    ...
    def page_info(self):
        ...
        return json.loads(value) if value else {
            ...
            "elementCount": 0, "scrollHeight": 0, "loginFormVisible": false,
            "html": ""
        }
...
class SiteScanner:
    def __init__(
        self, start_url, dynamic_cycles=None, dynamic_disabled=False,
    ...
    def enqueue_page(self, url, depth):
        ...
        if not url:
            return
        if url != self.start_url and is_facebook_noise_url(url):
            return
    ...
    def process_candidate(self, base_url, raw_url, depth, source, resource_hint=False):
        ...
        if not absolute:
            return
        redirect_target = facebook_redirect_target(absolute)
        if redirect_target:
            self.external_links.add(redirect_target)
            return
    ...
    def looks_like_login_page(self, info):
        page_url = normalize_url(info.get("url", "")) or ""
        if not self.is_known_dynamic_domain or not same_host(page_url, self.host):
            return False
        path = (urlparse(page_url).path or "").lower()
        if path.startswith(("/login", "/recover", "/reg", "/checkpoint")):
            return True
        if info.get("loginFormVisible"):
            return True
        title = (info.get("title") or "").lower()
        strong_title_markers = (
            "log in | facebook", "login | facebook", "entrar no facebook",
            "facebook login",
        )
        return any(marker in title for marker in strong_title_markers)

    def offer_interactive_login(self, url):
        """Offer a visible Chromium login without ever collecting credentials."""
        if not self.interactive_login or self.login_prompted:
            return None
        self.login_prompted = True
        print(
            "\n       [!] A sessão do perfil Chromium parece não estar autenticada.\n"
            "       O Firefox não compartilha cookies com o Chromium.\n"
            "       Posso abrir o Chromium visível para você fazer login manualmente.\n"
            "       O scanner não verá nem armazenará usuário ou senha."
        )
        try:
            choice = input("       Abrir Chromium para login? [s/N]: ").strip().lower()
        except (EOFError, KeyboardInterrupt):
            choice = ""
        if choice not in ("s", "sim", "y", "yes"):
            print("       [i] Login interativo ignorado; continuando com conteúdo público.")
            return None

        if not self.browser.relaunch_visible():
            print("       [!] Não foi possível abrir o Chromium visível para login.")
            return None
        info = self.browser.navigate(url)
        if not info:
            print("       [!] O Chromium não carregou a página de login.")
            return None
        print(
            "       [>] Faça login manualmente na janela do Chromium.\n"
        ...
        try:
            ...
            print()
        try:
            # O usuário pode ter sido redirecionado para a página inicial após
            # o login. Recarregue explicitamente a URL que originou o pedido.
            return self.browser.navigate(url)
        except Exception as exc:
            print(f"       [!] Não foi possível ler a página após o login: {exc}")
            return None
    ...
    def browser_fetch(self, url, depth, dynamic=False):
        ...
        if not same_host(final, self.host):
            self.external_links.add(final)
            print(f"       [>] Redirecionamento externo ignorado: {final}")
            return True
        if (
            not dynamic
        ...
        if self.looks_like_login_page(info):
            ...
            print("       [!] A página aparenta exigir login.")
            refreshed = self.offer_interactive_login(final)
            if refreshed:
                info = refreshed
                ...
                if not same_host(final, self.host):
                    ...
                    return True
                if self.looks_like_login_page(info):
                    print("       [!] A página ainda aparenta exigir login; continuando sem credenciais.")
                else:
                    self.login_required_pages.discard(login_url)
                    print("       [✓] Sessão Chromium autenticada; continuando a varredura.")
...
def main():
    ...
    parser.add_argument(
        "--no-login-prompt", action="store_true",
        help="não oferecer login manual no Chromium quando a página exigir autenticação",
    )
    ...
    try:
        scanner = SiteScanner(
            url, args.dynamic, args.no_dynamic,
